...
{
    // Let first check how many connection attempts have already occurred
    if(connectAttempts >= CONNECT_MAX_RETRIES)
    {
        // We've already retried the connection several times, so the problem is
        // unlikely to resovle itself. We'll just give up here
        Serial.println("Max connection attempts made.");
        freeze();
    }

    // We'll start a new connection attempt, so increment the counter
    connectAttempts++;
    ...
    // There was a problem connecting, send error code to error handler
    handleError(err);
}
...
void setup()
{
    ...
    Serial.println("If you're stuck, look for \"INTENTIONAL ERROR\" in this example's comments");
    Serial.println();
